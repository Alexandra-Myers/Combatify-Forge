...
public abstract class PlayerMixin extends LivingEntity implements PlayerExtensions, LivingEntityExtensions {
    ...
    public void injectCrit(Entity target, CallbackInfo ci, @Local(ordinal = 0) LocalFloatRef attackDamage, @Local(ordinal = 1) final float attackDamageBonus, @Local(ordinal = 2) LocalBooleanRef bl3) {
        ...
        if(!AtlasCombat.CONFIG.sprintCritsEnabled.get()) {
            isCrit &= !isSprinting();
        }
    ...
    public boolean attackSpeedsMaxed() {
        ForgeConfig c = AtlasCombat.CONFIG;
        UtilClass<Double> util = new UtilClass<>();
        return util.compare(7.5, c.swordAttackSpeed.get(), c.axeAttackSpeed.get(), c.woodenHoeAttackSpeed.get(), c.stoneHoeAttackSpeed.get(), c.ironHoeAttackSpeed.get(), c.goldDiaNethHoeAttackSpeed.get(), c.defaultAttackSpeed.get(), c.tridentAttackSpeed.get(), c.fastToolAttackSpeed.get(), c.fastestToolAttackSpeed.get(), c.slowToolAttackSpeed.get(), c.slowestToolAttackSpeed.get());
    }
}
